...
- Если нужны загрузки аватаров и обложек проектов, дополнительно задай S3-compatible storage переменные `MINIO_*`. Без этого основные сценарии работают, но media upload будет недоступен.
