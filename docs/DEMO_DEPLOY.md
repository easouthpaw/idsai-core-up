...
- В `render.yaml` email и фоновые задачи включены, чтобы работали регистрация, подтверждение email и сброс пароля.
- Для free web service на Render SMTP-порты недоступны, поэтому для писем нужно задать `RESEND_API_KEY` и `SMTP_FROM`. В этом режиме приложение отправляет письма через HTTPS API Resend.
- `SMTP_FROM` в режиме Resend используется как адрес отправителя и должен совпадать с sender'ом из verified domain в Resend.
- SMTP остаётся fallback-вариантом для локальной разработки или paid Render instance, но не подходит для free Render.
...
- Фоновые задачи и health monitor включены, поэтому email outbox и Telegram-алерты работают после деплоя.
...
- Форма контактов на landing page может доставлять сообщения через Telegram или напрямую на email. Для email fallback можно задать `CONTACT_EMAIL_TO`, иначе сервис возьмёт адрес из `SMTP_USER`, а если он пустой, то из адреса в `SMTP_FROM`.
